...
Original file is located at
    https://colab.research.google.com/drive/1RNNb_pcobaUjmudkNGICvQRFIfrSg8Z0
"""
...
train = pd.read_csv('/kaggle/input/dataset-layer11/train.csv').dropna()
valid = pd.read_csv('/kaggle/input/dataset-layer11/valid.csv').dropna()
test = pd.read_csv('/kaggle/input/dataset-layer11/test.csv')
...
cross_val_score(SVC(kernel='rbf', C=85, gamma='scale'), X_train, y_train['label_2'], cv=3).mean()

default_model = getSVM(SVC(kernel='rbf', C=85, gamma='scale'), X_train, y_train['label_2'])
y_pred = default_model.predict(X_valid)
accuracy_score(y_valid['label_2'], y_pred)
# accuracy when c=50 => 0.9171
# accuracy when c=75 => 0.9171
# accuracy when c=100,120, 150 => 0.9198
...
hyperparameterTune(X_train, y_train['label_4'])

cross_val_score(SVC(kernel='rbf', C=75), X_train, y_train['label_4'], cv=3).mean()

default_model = getSVM(SVC(kernel='rbf', C=75), X_train, y_train['label_4'])
y_pred = default_model.predict(X_valid)
...
df.to_csv("/kaggle/working/output_layer11.csv", index=False)
